...
使用 [全局变量](https://manual-en.yoyogames.com/GameMaker_Language/GML_Overview/Variables/Global_Variables.htm) 是使所有实例都可以访问控制器变量的好方法。然而，应该注意的是，引用它们的脚本调用（尤其是在编译到YYC时）可能会因全局变量的多次查找而减慢速度。例如，考虑以下脚本：
